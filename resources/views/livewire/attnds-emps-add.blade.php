                ...
                <div class="grid lg:grid-cols-2 gap-x-6 gap-y-3 px-12  justify-items-center sm:grid-cols-1 ">
                    <div class="flex flex-col space-y-1 col-span-2 w-full">
                        <div class="flex justify-between">
                          <label   class="labels">{{ __('att.emp') }}</label>
                          @error('emp') <span class="danger">{{ $message }}</span> @enderror  

                        </div>
                        <select    wire:model.defer="emp"   class="inputs w-full @error('emp') reds @enderror">
                          <option value="">-----</option>
                              @foreach ($Emps as $Emp)
