<div>
    <x-dialog-modal wire:model='visible' >
        <x-slot name='title'>
         ...
         <x-slot name='content'>
            <div x-data="{ open: 0 ,select: 0}" class="flex flex-col space-y-8 ">
      
                {{-- 4 top rows --}}
                <div class="grid lg:grid-cols-2 gap-x-6 gap-y-3 px-12  justify-items-center">
                      ...
                      <div class=" w-full flex flex-col space-y-2">
                        <div class="flex justify-between w-full ">
                            ...
                            @error('montant') <span class="danger">{{ $message }}</span> @enderror  
                        </div>
                        <input dir="ltr" wire:model='montant' class="inputs w-full @error('montant') reds @enderror" type="text" x-mask:dynamic="$money($input, '.', ' ')"   required  /> 
                      </div>  
                      ...
                      <div class=" flex space-x-2 items-center w-full">
                        <input value="true" id="{{ $fix }}"  wire:model='fix' type="checkbox" @if($fix) checked @endif class="check w-6 h-6" >
                          <label for="{{ $fix }}"  class="flex items-center">
