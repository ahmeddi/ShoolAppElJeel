<div>
    <div class=" flex justify-between dark:border-gray-600 p-2">
        ...
        @can('sur_or_dir')
        <button x-on:click="jorn()" class="print:hidden bg-gray-200 p-2 rounded-md hover:bg-teal-200 text-teal-800 ">
    ...
    <div class="w-full hidden print:block lg:block md:block">
        <table class="w-full ">
            <thead>
                <tr>
                                ...
                                @if ($loop->first)
                                    <td class="border print:border-gray-900 dark:bg-gray-800 dark:border-gray-600 border-gray-300 bg-gray-100 text-gray-800 p-1 h-40   ">
                                        <div class="top h-5 w-full">
                                            <span class="text-gray-500 dark:text-gray-300 font-medium print:text-gray-900 dark:print:text-gray-900">{{ $Time }}</span>
                                        </div>
                                    </td>
                                @endif   
                                <td class="border dark:bg-gray-800 dark:border-gray-600 print:border-gray-900 p-1 h-40 w-10 relative ">   
                                                @can('dir')
                                                <button wire:click="$dispatchTo('add-calandar-c','open',{ id1: {{ $loop->index  }}, id2 : {{ $loop->parent->index }} })"  class="absolute top-1 right-1 print:hidden">
                                            ...
                                            @forelse ($datas as $data)
                                                @if ($data->time == $loop->parent->parent->index and $data->day == $loop->parent->index)   
                                                @can('dir')
                                                <button wire:key='{{ $data->id }}'  wire:click="$dispatchTo('c-clandar-del','open',{ id: {{ $data->id  }} })"  class="absolute top-1 left-1 stroke-red-600 print:hidden">
    ...
    <div class="w-full  lg:hidden md:hidden print:hidden">
                        ...
                        <tr class="bg-gray-50 border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <th scope="row" class="px-6 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    @foreach ($datas as $data)
                                        @if ($data->time == ($index -1) and $data->day == ($dayIndex -1))
                                            <div class="my-1 font-bold text-sm print:text-xs text-teal-600 dark:text-teal-300 print:text-gray-900 dark:print:text-gray-900">
                                                {{ $data->mat->nom }}
                                            </div>
                                        @endif
