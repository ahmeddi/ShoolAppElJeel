...
class WhatsappApiService
{
    ...
    function sendCurlRequestForward($phone, $id)
    {
        $profil = Profil::find(1);
        $url = 'https://gate.whapi.cloud/messages/'.$id;
        $token = $profil->token;
    ...
    function sendCurlImage($phone, $img)
    {
        $profil = Profil::find(1);
        $token = $profil->token;
        ...
        curl_setopt_array($curl, [
          CURLOPT_URL => "https://gate.whapi.cloud/messages/image",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => "",
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 30,
          ...
          CURLOPT_POSTFIELDS => json_encode([
            'to' => $phone,
            'media' => "data:image/png;name=FC_Barcelona_(crest).svg.png;base64,$img",
            'caption' => 'aaa',
         //   'quoted' => 'bbb',
            ]),
          CURLOPT_HTTPHEADER => [
    ...
    function sendCurlRequestFirst($phone, $message)
    {
        $profil = Profil::find(1);
        $url = $profil->url;
        $token = $profil->token;
